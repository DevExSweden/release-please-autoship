...
4. Calls the internal `auto-merge-pr` action to approve and enable auto-merge on the release PR.
  ...
  release:
    ...
    steps:
      - name: Run Release Please and auto-merge
        uses: sitoo/mobile-shared-cicd/actions/release-please-autoship@main
        with:
          target-branch: main
...
googleapis/release-please-action  ← creates/updates release PR
        ...
        └─ prs_created == 'true'
         ...
         Extract PR number (jq)
                │
                ▼
         auto-merge-pr             ← GITHUB_TOKEN approves, App token enables auto-merge
```
