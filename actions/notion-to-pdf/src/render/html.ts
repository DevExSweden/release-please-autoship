...
export function renderNodesToHtml(nodes: BlockNode[]): string {
  ...
  while (i < nodes.length) {
    ...
    if (t === "paragraph") {
      ...
      const renderRow = (rowNode: BlockNode, cellTag: "td" | "th", rowHeader: boolean) => {
        const cells = (rowNode.block.table_row?.cells ?? []) as RichText[][];
        const cellHtml = cells
          .map((cell, idx) => {
            const tag = rowHeader && idx === 0 ? "th" : cellTag;
            return `<${tag}>${renderRichText(cell as any)}</${tag}>`;
          })
          .join("");
